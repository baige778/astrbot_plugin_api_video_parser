...
- ✏️ **自定义提示语** — 处理中提示、视频已删除提示均可自定义，留空即不发送
...
| `untitled_title` | string | `未命名` | 无标题时的回退文案（可自定义） |
| `unknown_author` | string | `未知作者` | 无作者时的回退文案（可自定义） |
...
4. 图集：逐张下载转 base64 发送（绕过防盗链）
5. 视频：探测文件大小 → 检查上限 → 发送封面图（可选）→ 发送视频直链
6. 视频已删除：发送自定义提示语
