...
- 📚 **图集合并发送** — 图集总张数超过阈值时，将所有图片合并成一条消息发送（阈值可配置，设为 0 则始终逐张）
- 🎞️ **视频合并发送** — 可开启将封面图、标题作者与视频合并为一条消息发送（开关控制）
...
| `album_merge_threshold` | int | `9` | 图集合并阈值（张），总张数超过该值则将全部图片合并成一条消息；设为 `0` 表示始终逐张发送 |
| `video_merge_message` | bool | `false` | 是否将封面图/标题作者/视频合并成一条消息发送（关闭则封面与视频分开发送） |
...
4. 图集：逐张下载转 base64 发送（绕过防盗链）；总张数超过合并阈值时，将所有图片合并成一条消息
5. 视频：探测文件大小 → 检查上限 → 发送封面图（可选）→ 发送视频直链；可配置将封面/标题/视频合并为一条消息
6. 视频已删除：发送自定义提示语
