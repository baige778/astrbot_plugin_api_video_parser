...
- 🎞️ **视频合并发送** — 可开启将封面图、标题作者与视频以「合并转发」形式合并成一条消息发送（开关控制）；视频超过 100MB 时自动改用「文件」节点合并转发，绕开 NapCat 对转发视频的 100MB 上传限制
- 🔐 **群聊黑白名单** — 提供 Plugin Page 自定义页面，自动读取机器人加入的 QQ 群，可视化配置黑名单/白名单，支持私聊消息单独开关
- 🔔 **登录态失效提醒** — 抖音登录态失效时代替报错，自动提示用户重新发送 `/dy登陆` 扫码登录
...
v0.3.22 起提供 AstrBot Plugin Page 自定义页面。在 WebUI 插件详情页打开「视频解析 · 群聊黑白名单」即可：

- **抖音登录状态** — 实时显示解析后端是否已登录抖音，未登录时提示发送 `/dy登陆` 扫码登录。
- **群聊黑白名单** — 自动读取机器人加入的 QQ 群列表，可视化勾选：
  - `启用群聊黑白名单`：总开关，关闭则所有会话均可解析。
  - `过滤模式`：黑名单（勾选的群禁止解析）或白名单（仅勾选的群允许解析）。
  - `私聊消息允许解析`：独立控制私聊（非群聊）是否可触发解析。
...
├── main.py                # 核心逻辑：平台正则、API 调用、图集/视频处理、黑白名单、Web API
